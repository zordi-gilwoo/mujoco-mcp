...
from py_remote_viewer.events import (
    EventProtocol,
    EventType,
    MouseEvent,
    KeyEvent,
...
class TestSignalingServer:
    ...
    async def test_event_processing(self):
        ...
        mouse_msg = {"type": "mouse_move", "x": 100, "y": 200, "buttons": 1}

        # Should not raise exceptions
        await server._process_event_message(mouse_msg)

        # Test keyboard event
        key_msg = {"type": "key_down", "code": "Space", "ctrl": False, "alt": False, "shift": False}

        await server._process_event_message(key_msg)

        # Test command event
        cmd_msg = {"type": "command", "cmd": "pause", "params": {}}
...
class TestIntegration:
    ...
    async def test_scene_loading_api(self):
        ...
        async with httpx.AsyncClient(app=app, base_url="http://test") as client:
            ...
            response = await client.post("/api/scene/load", json={"xml": scene_xml})
