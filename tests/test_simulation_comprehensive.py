...
from mujoco_mcp.simulation import MuJoCoSimulation, EnhancedMuJoCoSimulation
...
class TestMuJoCoSimulation:
    ...
    @patch('mujoco_mcp.enhanced_simulation.mujoco')
    def test_load_model_success(self, mock_mujoco, temp_model_file):
        ...
        # Test
        sim = MuJoCoSimulation()
        sim.load_model(temp_model_file)
    ...
    def test_load_model_file_not_found(self, mock_mujoco):
        ...
        with pytest.raises(FileNotFoundError):
            sim.load_model("non_existent_file.xml")
    ...
    def test_set_joint_positions(self, mock_mujoco):
    ...
    def test_set_joint_positions_wrong_size(self, mock_mujoco):
        ...
        assert "Expected 3 joint positions" in str(exc_info.value)
    ...
    def test_apply_control_wrong_size(self, mock_mujoco):
        ...
        assert "Expected 2 control inputs" in str(exc_info.value)
    ...
    def test_get_sensor_data(self, mock_mujoco):
        ...
        assert 'raw_data' in sensor_data
        np.testing.assert_array_equal(sensor_data['raw_data'], 
