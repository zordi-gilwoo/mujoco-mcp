...
class SceneXMLBuilder:
    ...
    def build_scene(
        ...
        poses: Dict[str, Pose],
        scene_name: str = "structured_scene"
    ) -> str:
        """
        ...
        for obj in scene.objects:
            if obj.object_id in poses:
                self._add_object_to_worldbody(
                    worldbody, obj.object_id, obj.object_type, poses[obj.object_id]
                )
            else:
        ...
        for robot in scene.robots:
            if robot.robot_id in poses:
                self._add_robot_to_worldbody(
                    worldbody, robot, poses[robot.robot_id]
                )
            else:
    ...
    def _add_object_to_worldbody(
        ...
        """Add an object to the worldbody."""
    ...
    def _configure_robot_joints(self, robot_element: ET.Element, robot_config, metadata: AssetMetadata):
        ...
        if robot_config.custom_joints:
            ...
            if robot_config.joint_config in metadata.joint_configs:
                ...
                logger.debug(f"Using named joint config '{robot_config.joint_config}' for {robot_config.robot_id}")
