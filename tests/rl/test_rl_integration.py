...
class RLIntegrationTest:
    ...
    def test_environment_structure(self, env):
        ...
        dummy_action = np.zeros(
            env.action_space.shape[0] if hasattr(env.action_space, "shape") else 3
        )
    ...
    def test_action_space_compatibility(self):
        ...
        try:
            ...
            print(
                f"   ✅ Continuous action: shape={action_cont.shape}, range=[{action_cont.min():.2f}, {action_cont.max():.2f}]"
            )

            # Test discrete action space
            config_discrete = RLConfig(
                robot_type="cart_pole", task_type="balancing", action_space_type="discrete"
            )
    ...
    def test_reward_functions(self):
        ...
        try:
            ...
            reward_balance = env_balance.reward_function.compute_reward(
                obs_balance, action_balance, obs_balance, {}
            )
            ...
            reward_walk = env_walk.reward_function.compute_reward(
                obs_walk, action_walk, obs_walk, {}
            )
