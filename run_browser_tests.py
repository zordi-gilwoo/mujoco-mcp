...
def check_dependencies():
    ...
    try:
        ...
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "playwright", "pytest-playwright", "pytest-asyncio", "--user"])
    ...
    try:
        ...
        if "is already installed" in result.stdout or result.returncode == 0:
            ...
            try:
                ...
                subprocess.check_call([sys.executable, "-m", "playwright", "install", "chromium", "--force"])
...
async def run_simple_ui_tests():
    ...
    if html_file.exists():
        ...
        print(f"   {'✅' if ui_tests['html_structure_valid'] else '❌'} HTML structure valid: {ui_tests['html_structure_valid']}")
    ...
    if js_file.exists():
        ...
        print(f"   {'✅' if ui_tests['js_file_valid'] else '❌'} JavaScript file valid: {ui_tests['js_file_valid']}")
    ...
    if css_file.exists():
        ...
        print(f"   {'✅' if ui_tests['css_file_valid'] else '❌'} CSS file valid: {ui_tests['css_file_valid']}")
...
async def run_integration_tests():
    ...
    integration_tests = {
        "server_web_integration": False,
        "command_execution_simulation": False
    }
    ...
    try:
        ...
        print(f"   {'✅' if success_count > 0 else '❌'} Command execution simulation: {success_count}/{len(commands_to_test)} commands succeeded")
...
async def main():
    ...
    try:
        ...
        has_any_success = any(
            isinstance(results, dict) and any(results.values())
