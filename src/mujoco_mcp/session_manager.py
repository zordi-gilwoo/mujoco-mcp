...
class SessionManager:
    ...
    def get_or_create_session(self, context: Any = None) -> SessionInfo:
        ...
        with self._lock:
            ...
            logger.info(f"Created new session {session_id} for client {client_id} on port {viewer_port}")
            return session
    ...
    def get_viewer_client(self, context: Any = None) -> Optional[MuJoCoViewerClient]:
        ...
        # Use ViewerManager to get client for this session's models
        # For simplicity, we'll use the session_id as model_id for the primary client
        ...
        if not client:
            ...
            if success:
                client = viewer_manager.get_client(session.session_id)
                logger.info(f"Created viewer client for session {session.session_id}")
            else:
                logger.error(f"Failed to create viewer client for session {session.session_id}")
                return None
