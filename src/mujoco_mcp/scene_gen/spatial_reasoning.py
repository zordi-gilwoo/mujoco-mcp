...
class RobotReachabilityChecker:
    ...
    def check_reachability(
        self, robot_type: str, robot_pose: Pose, target_position: np.ndarray
    ) -> Tuple[bool, float]:
        """
    ...
    def find_optimal_base_position(
        self,
        robot_type: str,
    ...
    def _get_entity_metadata(
        self, entity_id: str, scene: SceneDescription
...
class AdvancedSpatialReasoner:
    ...
    def enhance_object_placement(
        ...
        if stable_pose is None:
            logger.debug(
                f"No stable pose found for {obj.object_type}; retaining solver placement"
            )
            if initial_pose is not None:
                return initial_pose, 0.5
            basic_pose = Pose(
        ...
        logger.debug(
            f"Enhanced placement for {obj.object_id}: stability={stable_pose.stability_score:.2f}"
        )

        return enhanced_pose, stable_pose.stability_score

    def validate_robot_workspace(
        self, robot: RobotConfiguration, scene: SceneDescription, existing_poses: Dict[str, Pose]
        ...
        if unreachable_targets:
            ...
            suggestions.append(
                f"Suggested base position: [{optimal_pos[0]:.2f}, {optimal_pos[1]:.2f}, {optimal_pos[2]:.2f}]"
            )
    ...
    def suggest_workspace_improvements(
            ...
            if obj.object_id in poses:
                ...
                if stable_poses:
                    ...
                    if max_stability < 0.7:
                        suggestions.append(
                            f"Object {obj.object_id} may be unstable (max stability: {max_stability:.2f})"
                        )
