...
class TestRobotReachabilityChecker:
    ...
    def test_reachability_checking(self):
        ...
        robot_pose = Pose(position=np.array([0, 0, 0]), orientation=np.array([0, 0, 0, 1]))
    ...
    def test_optimal_base_position_finding(self):
        ...
        # Create simple scene
        scene_dict = {
            "objects": [{"object_id": "table1", "object_type": "table_standard", "constraints": []}]
        }
        scene = SceneDescription(**scene_dict)

        # Existing poses (just the table)
        existing_poses = {
            "table1": Pose(position=np.array([1, 0, 0]), orientation=np.array([0, 0, 0, 1]))
        }
...
class TestAdvancedSpatialReasoner:
    ...
    def test_enhanced_object_placement(self):
        ...
        # Object to place
        obj = ObjectPlacement(object_id="test_cup", object_type="cup_ceramic_small", constraints=[])

        # Support surface (table)
        support_metadata = self.metadata_extractor.get_metadata("table_standard")
        support_pose = Pose(position=np.array([0, 0, 0]), orientation=np.array([0, 0, 0, 1]))

        enhanced_pose, stability_score = reasoner.enhance_object_placement(
            obj, support_metadata, support_pose, initial_pose=None
        )
    ...
    def test_workspace_validation(self):
        ...
        scene_dict = {
            "objects": [
                {"object_id": "table1", "object_type": "table_standard", "constraints": []},
                {"object_id": "cup1", "object_type": "cup_ceramic_small", "constraints": []},
            ],
            "robots": [
                {
                    "robot_id": "robot1",
        ...
        is_valid, score, suggestions = reasoner.validate_robot_workspace(robot_config, scene, poses)
