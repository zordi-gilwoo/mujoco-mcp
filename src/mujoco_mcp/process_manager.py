...
class ProcessManager:
    ...
    def __init__(
        ...
        logger.info(
            f"ProcessManager initialized with port range {port_range_start}-{port_range_end}"
    ...
    def spawn_viewer_process(self, session_id: str, isolated: bool = True) -> Optional[ProcessInfo]:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.info(
                    f"Spawned viewer process for session {session_id} on port {port} (PID: {process.pid})"
                )
                ...
                if not process_info.is_running:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Process for session {session_id} died immediately (timeout getting output)"
                        )
...
process_manager = ProcessManager(port_range_start=8001, port_range_end=9000, use_mock_server=False)
...
test_process_manager = ProcessManager(
    port_range_start=8001, port_range_end=9000, use_mock_server=True
)
