...
class RLEnvironmentRunner:
    ...
    async def load_environment_to_viewer(self, xml_content: str, model_id: str = "rl_environment") -> bool:
    ...
    async def start_random_actions(self, num_steps: int = 1000, step_delay: float = 0.02) -> bool:
        ...
        self.runner_task = asyncio.create_task(
            self._run_random_actions_loop(num_steps, step_delay)
        )
        ...
        try:
            ...
            for step in range(num_steps):
                ...
                if terminated or truncated:
                    ...
                    self.status.average_reward = np.mean(episode_rewards) if episode_rewards else 0.0
                    
                    logger.info(f"Episode {self.status.current_episode} finished: reward={current_episode_reward:.3f}")
            ...
            logger.info(f"Completed {num_steps} steps across {self.status.current_episode} episodes")
    ...
    async def _update_viewer_state(self, observation: np.ndarray, action: np.ndarray, reward: float):
