...
async def simulate_client(
    client_id: str, scene_type: str, session_manager: SessionManager
) -> List[str]:
    ...
    try:
        ...
        if client:
            ...
            if process_info:
                results.append(
                    f"   📈 Process PID: {process_info.pid}, Running: {process_info.is_running}"
                )
        else:
...
def run_client_in_thread(
    client_id: str, scene_type: str, session_manager: SessionManager, results_dict: dict
):
...
async def demo_sequential_clients():
    ...
    for session_id, session_info in stats["sessions"].items():
        ...
        if session_info.get("process"):
            ...
            print(
                f"     - Process: PID={proc['pid']}, Port={proc['port']}, Running={proc['is_running']}"
            )
...
async def demo_concurrent_clients():
    ...
    for client_id, scene_type in concurrent_tests:
        thread = threading.Thread(
            target=run_client_in_thread, args=(client_id, scene_type, session_manager, results_dict)
        )
