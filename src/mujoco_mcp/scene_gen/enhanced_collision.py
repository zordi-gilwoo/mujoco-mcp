...
def transform_bbox_with_rotation(
    bbox_min: np.ndarray, bbox_max: np.ndarray, pose: Pose
...
class GlobalCollisionOptimizer:
    ...
    def optimize_poses(
        self, scene: SceneDescription, initial_poses: Dict[str, Pose]
    ) -> Dict[str, Pose]:
        """
        ...
        for iteration in range(self.max_iterations):
            ...
            logger.debug(
                f"Iteration {iteration}: moved {len(movement_vectors)} entities, total movement: {total_movement:.4f}m"
            )
        ...
        if self.physics_validator.physics_available:
            ...
            if physics_collisions:
                logger.warning(
                    f"Physics validation still shows {len(physics_collisions)} contacts after optimization"
                )
    ...
    def _detect_all_collisions(
        ...
        poses: Dict[str, Pose],
        collision_constraints: List[SpatialConstraint],
    ) -> List[CollisionInfo]:
        ...
        for constraint in collision_constraints:
            ...
            reference_aabb = RotationAwareAABB.from_metadata(
                reference_metadata, poses[reference_id]
            )
    ...
    def _compute_global_movement_vectors(
        self,
        scene: SceneDescription,
        poses: Dict[str, Pose],
        ...
        for collision in collisions:
            ...
            for constraint in collision_constraints:
                if (
                    constraint.subject == collision.entity1_id
                    and constraint.reference == collision.entity2_id
                ) or (
                    constraint.subject == collision.entity2_id
                    and constraint.reference == collision.entity1_id
                ):
            ...
            if separation_magnitude > 0:
                ...
                required_movement = (
                    collision.separation_vector
                    / separation_magnitude
                    * (separation_magnitude + clearance)
                )
                ...
                if total_mobility > 0:
                    ...
                    movement_vectors[collision.entity1_id] += (
                        required_movement * (mobility1 / total_mobility) * 0.5
                    )
                    # Entity2 moves in negative separation direction
                    movement_vectors[collision.entity2_id] -= (
                        required_movement * (mobility2 / total_mobility) * 0.5
                    )
