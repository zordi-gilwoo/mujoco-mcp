...
class MultiProviderLLMDemo:
    ...
    def display_provider_config(self, provider: str):
        ...
        config_vars = {
            ...
            'claude': ['CLAUDE_API_KEY', 'ANTHROPIC_API_KEY', 'CLAUDE_MODEL', 'CLAUDE_TEMPERATURE', 'CLAUDE_MAX_TOKENS'],
            'gemini': ['GEMINI_API_KEY', 'GOOGLE_API_KEY', 'GEMINI_MODEL', 'GEMINI_TEMPERATURE', 'GEMINI_MAX_TOKENS']
        }
        ...
        try:
            ...
            if api_key:
                ...
                if not self.llm_generator.api_key:
                    print(f"⚠️  No API key found for {provider}. Set environment variable or pass key directly.")
                    return False
            ...
            for i, obj in enumerate(scene.objects, 1):
                ...
                if obj.constraints:
                    for constraint in obj.constraints:
                        print(f"      - {constraint.type}: {constraint.subject} → {constraint.reference}")
    ...
    def test_fallback_system(self):
        ...
        try:
            test_prompt = "Create a complex manipulation scenario with multiple objects and constraints"
            ...
            print(f"📊 Fallback scene contains {len(scene.objects)} objects and {len(scene.robots)} robots")
    ...
    def display_summary(self):
        ...
        if successful_tests:
            ...
            for test in successful_tests:
                print(f"   • {test['provider'].upper()}: {test['time']:.2f}s, {test['objects']} objects, {test['robots']} robots")
    ...
    async def run_demo(self):
        ...
        for provider, display_name in providers_to_test:
            ...
            api_key = None
            for var in (api_key_vars[provider] if isinstance(api_key_vars[provider], list) else [api_key_vars[provider]]):
            ...
            if api_key:
                ...
                print(f"   Set environment variable: {api_key_vars[provider][0] if isinstance(api_key_vars[provider], list) else api_key_vars[provider]}")
