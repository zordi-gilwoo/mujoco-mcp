...
class PlanValidator:
    ...
    def validate_plan(self, plan: SymbolicPlan) -> Tuple[bool, List[str], Dict[str, Any]]:
        ...
        logger.info(
            f"Plan validation completed: {'VALID' if is_valid else 'INVALID'} ({len(errors)} errors)"
        )
        ...
        for op in plan.operations:
            for dep in op.dependencies:
                if dep not in all_entities:
                    errors.append(
                        f"Operation {op.target_entity} depends on non-existent entity: {dep}"
                    )
                ...
                if constraint_type in ["on_top_of", "beside", "in_front_of"]:
                    if reference in spatial_refs:
                        ...
                        if prev_type != constraint_type:
                            errors.append(
                                f"Conflicting spatial constraints for {entity} relative to {reference}: {prev_type} vs {constraint_type}"
                            )
        ...
        for op in plan.operations:
            if op.operation_type == OperationType.PLACE_OBJECT:
                ...
                if object_type:
                    ...
                    if metadata is None:
                        errors.append(
                            f"Unknown object type: {object_type} for entity {op.target_entity}"
                        )
                ...
                if robot_type:
                    ...
                    if metadata is None:
                        errors.append(
                            f"Unknown robot type: {robot_type} for entity {op.target_entity}"
                        )
        ...
        for op in place_operations:
            for constraint in op.constraints:
                if constraint.get("type") == "on_top_of":
                    ...
                    if ref_op is None:
                        errors.append(
                            f"Object {op.target_entity} placed on_top_of unknown reference: {reference}"
                        )
                    else:
                        ...
                        if ref_type and "table" not in ref_type.lower():
                            ...
                            logger.warning(
                                f"Object {op.target_entity} placed on {reference} which may not be a good support surface"
                            )
    ...
    def _validate_safety(self, plan: SymbolicPlan) -> Tuple[bool, List[str]]:
        ...
        for robot_op in robot_operations:
            ...
            if not robot_has_clearance:
                warnings.append(
                    f"Robot {robot_op.target_entity} may not have adequate clearance from objects"
                )
        ...
        for obj1, obj2 in object_pairs:
            if (obj1, obj2) not in collision_constraints and (
                obj2,
                obj1,
            ) not in collision_constraints:
                missing_collision_checks.append((obj1, obj2))

        if missing_collision_checks:
            warnings.append(
                f"Missing explicit collision avoidance for {len(missing_collision_checks)} object pairs"
            )
...
class SymbolicPlanGenerator:
    ...
    def generate_plan_from_nl(
        self, natural_language: str, plan_id: Optional[str] = None
    ) -> SymbolicPlan:
        """
        ...
        plan = SymbolicPlan(
            plan_id=plan_id, description=f"Generated from: {natural_language[:100]}..."
        )
...
class PlanToSceneConverter:
    ...
    def convert_plan_to_scene(self, plan: SymbolicPlan) -> SceneDescription:
        ...
        scene = SceneDescription(
            objects=objects, robots=robots, workspace_bounds=plan.workspace_bounds
        )

        logger.info(
            f"Converted plan {plan.plan_id} to scene with {len(objects)} objects and {len(robots)} robots"
        )

        return scene

    def _sort_operations_by_dependencies(
        self, operations: List[SymbolicOperation]
    ) -> List[SymbolicOperation]:
