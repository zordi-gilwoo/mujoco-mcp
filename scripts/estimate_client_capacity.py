...
def calculate_capacity_limits(system_info: Dict[str, Any], usage_estimates: Dict[str, Any]) -> Dict[str, Any]:
...
def generate_recommendations(capacity_limits: Dict[str, Any], system_info: Dict[str, Any]) -> list:
    ...
    if limiting_factor == "memory_limited":
        ...
        recommendations.append(f"💾 Memory is the limiting factor. Consider adding more RAM (current: {memory_gb}GB)")
    ...
    if limiting_factor == "cpu_limited":
        ...
        recommendations.append(f"🚀 CPU is the limiting factor. Consider more CPU cores (current: {cpu_cores})")
    ...
    if limiting_factor == "mujoco_limited":
        ...
        recommendations.append("🎮 Consider using headless mode for some clients to reduce viewer load")
        recommendations.append("🎮 Implement viewer connection pooling or sharing")
    
    if max_clients < 10:
        recommendations.append("⚠️  Very low capacity detected. System may not be suitable for multi-client deployment")
...
def main():
    ...
    if args.output == "json":
        ...
        if system_info.get("memory"):
            ...
            print(f"Memory: {mem['available_gb']}GB available / {mem['total_gb']}GB total ({mem['percent_used']}% used)")
