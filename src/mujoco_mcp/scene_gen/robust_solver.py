...
class BacktrackingSearchNode:
    ...
    def __init__(
        self, entity_id: str, pose: Pose, parent: Optional["BacktrackingSearchNode"] = None
    ):
...
class RobustConstraintSolver:
    ...
    def solve_robust(self, scene: SceneDescription) -> Tuple[Dict[str, Pose], Dict[str, Any]]:
        ...
        """
        logger.info(
            f"Starting robust constraint solving for {len(scene.get_all_entity_ids())} entities"
        )
    ...
    def _analyze_constraint_conflicts(self, scene: SceneDescription) -> List[ConstraintConflict]:
        ...
        for entity_id in all_entities:
            constraints = scene.get_constraints_for_entity(entity_id)
            spatial_constraints = [
                c for c in constraints if c.type in ["on_top_of", "in_front_of", "beside"]
            ]
    ...
    def _constraints_potentially_conflict(
        self, c1: SpatialConstraint, c2: SpatialConstraint
    ) -> bool:
    ...
    ) -> Tuple[Optional[GlobalSceneState], Dict[str, Any]]:
        """Perform backtracking search to find valid scene layout."""
        stats = {"search_nodes_explored": 0, "backtrack_events": 0, "max_depth_reached": 0}
    ...
    def _sort_entities_by_complexity(
        self, scene: SceneDescription, entities: List[str]
    ) -> List[str]:
    ...
    def _generate_alternative_poses(
        self, entity_id: str, scene: SceneDescription, existing_poses: Dict[str, Pose]
    ) -> List[Pose]:
        ...
        if not constraints:
            ...
            for x in np.linspace(-1, 1, 3):
                for y in np.linspace(-1, 1, 3):
                    alternatives.append(
                        Pose(position=np.array([x, y, 0]), orientation=np.array([0, 0, 0, 1]))
                    )
        else:
            # Constrained entity - generate poses based on constraints
            base_pose = self._compute_constraint_based_pose(
                entity_id, constraints, existing_poses, scene
            )
            if base_pose:
    ...
    def _compute_constraint_based_pose(
        self,
        entity_id: str,
        constraints: List[SpatialConstraint],
        existing_poses: Dict[str, Pose],
    ...
    def _is_placement_feasible(
        self, entity_id: str, pose: Pose, scene: SceneDescription, current_poses: Dict[str, Pose]
    ) -> bool:
    ...
    def _is_constraint_satisfied(
        self, constraint: SpatialConstraint, poses: Dict[str, Pose], scene: SceneDescription
    ) -> bool:
    ...
    def _calculate_collision_avoidance_score(
        self, scene: SceneDescription, poses: Dict[str, Pose]
    ) -> float:
    ...
    def _calculate_spatial_quality_score(
        self, scene: SceneDescription, poses: Dict[str, Pose]
    ) -> float:
    ...
    def _multi_pass_refinement(
        self, scene: SceneDescription, initial_solution: GlobalSceneState
    ) -> Tuple[GlobalSceneState, Dict[str, Any]]:
        ...
        for pass_num in range(self.max_refinement_passes):
            ...
            logger.debug(
                f"Refinement pass {pass_num + 1}: score improved by {score_improvement:.4f}"
            )

        return current_solution, refinement_stats

    def _local_improvement_pass(
        self, scene: SceneDescription, current_solution: GlobalSceneState
    ) -> GlobalSceneState:
