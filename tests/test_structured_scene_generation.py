...
class TestSceneSchema:
    ...
    def test_spatial_constraint_validation(self):
        ...
        constraint = SpatialConstraint(
            type="on_top_of", subject="cup_1", reference="table_1", clearance=0.01
        )
        ...
        with pytest.raises(ValueError, match="Constraint type must be one of"):
            SpatialConstraint(type="invalid_type", subject="cup_1", reference="table_1")

    def test_object_placement_validation(self):
        """Test object placement validation."""
        # Valid object placement
        obj = ObjectPlacement(object_id="table_1", object_type="table_standard", constraints=[])
        ...
        with pytest.raises(ValueError, match="Object ID cannot be empty"):
            ObjectPlacement(object_id="", object_type="table_standard")
...
class TestSceneXMLBuilder:
    ...
    def test_xml_generation(self):
        ...
        scene_dict = {
            "objects": [
                {"object_id": "table_1", "object_type": "table_standard", "constraints": []},
                {"object_id": "cup_1", "object_type": "cup_ceramic_small", "constraints": []},
            ]
        }
        ...
        poses = {
            ...
            "cup_1": Pose(
                position=np.array([0.0, 0.0, 0.8]), orientation=np.array([0.0, 0.0, 0.0, 1.0])
            ),
        }
...
class TestLLMSceneGenerator:
    ...
    def test_canned_example_generation(self):
        ...
        for prompt, expected_entities in prompts_and_expected:
            ...
            assert (
                len(entity_ids) == expected_entities
            ), f"Expected {expected_entities} entities for '{prompt}', got {len(entity_ids)}: {entity_ids}"
