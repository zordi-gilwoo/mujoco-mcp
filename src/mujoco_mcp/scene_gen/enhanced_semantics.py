...
class EnhancedAssetMetadata(AssetMetadata):
    ...
    def get_grasp_affordances(self, grasp_type: Optional[GraspType] = None) -> List[GraspAffordance]:
    ...
    def get_best_grasp_affordance(self, grasp_type: Optional[GraspType] = None) -> Optional[GraspAffordance]:
    ...
    def get_support_surfaces(self, surface_type: Optional[SurfaceType] = None,
                           primary_only: bool = False) -> List[SupportSurfaceInfo]:
...
class EnhancedAssetDatabase:
    ...
    def _load_enhanced_database(self):
        ...
        try:
            if Path(self.database_path).exists():
                ...
                logger.info(f"Loaded {len(self.enhanced_assets)} enhanced assets from {self.database_path}")
            else:
                logger.warning(f"Enhanced asset database not found at {self.database_path}, creating default assets")
    ...
    def _create_enhanced_metadata(self, asset_id: str, asset_data: Dict[str, Any]) -> EnhancedAssetMetadata:
        ...
        if 'robot_mounting_rules' in asset_data:
            ...
            robot_mounting_rules = RobotMountingRule(
                ...
                preferred_orientations=[np.array(orient) for orient in rules_data['preferred_orientations']],
    ...
    def find_compatible_support(self, object_mass: float, required_stability: float = 0.8) -> List[EnhancedAssetMetadata]:
