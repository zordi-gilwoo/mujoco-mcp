...
class MockBrowserSession:
    ...
    def __init__(self, server_url="http://localhost:8080"):
        ...
        self.session_data = {"command_history": [], "results_history": [], "ui_interactions": []}
    ...
    async def simulate_command_execution(self, command: str):
        ...
        if (
            not self.session_data["command_history"]
            or self.session_data["command_history"][-1] != command
        ):
...
async def test_mock_user_interactions():
    ...
    test_commands = ["get server info", "create pendulum simulation", "show current state"]
...
async def test_full_integration_workflow():
    ...
    successful_steps = sum(
        1 for result in results.values() if result and not str(result).startswith("Error:")
    )

    assert successful_steps >= 2, f"Too many workflow steps failed. Results: {results}"

    print(
        f"✅ Full integration workflow test passed ({successful_steps}/{len(workflow_steps)} steps successful)"
    )
