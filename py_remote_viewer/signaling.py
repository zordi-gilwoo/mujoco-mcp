...
class SignalingServer:
    ...
    async def _handle_offer(self, client_id: str, message: Dict):
        ...
        async def on_icecandidate(candidate):
            if candidate:
                await self._send_to_client(
                    client_id, {"type": "ice-candidate", "candidate": candidate.to_dict()}
                )
        ...
        await self._send_to_client(client_id, {"type": "answer", "sdp": pc.localDescription.sdp})
    ...
    async def _metrics_reporter(self):
        ...
        while True:
            ...
            logger.info(
                f"[Metrics] Connected clients: {self.stats['clients_connected']}, "
        ...
        async with self._command_lock:
            ...
            try:
                if any(keyword in command_lower for keyword in ("play", "start", "resume")):
                    ...
                    if self.llm_generator and any(
                        keyword in command_lower
                        for keyword in ("create", "generate", "make", "build")
                    ):
                        try:
                            ...
                            if scene_result.get("success") and scene_result.get("scene_xml"):
                                ...
                                return {
                                    "success": False,
                                    "error": scene_result.get("error", "Failed to generate scene"),
                                }
                        except Exception as e:
                    ...
                    if builtin_scene:
                        if builtin_scene == "franka_panda":
                            success, scene_xml, scene_path = self._load_menagerie_scene(
                                "franka_emika_panda"
                            )
                            if not success:
                                return {"success": False, "error": scene_path}

                            # Scene already loaded by _load_menagerie_scene
                            if scene_xml:
                                # Broadcast to clients (simulation already loaded)
                                await self.broadcast_scene_update(
                                    scene_xml, load_into_simulation=False
                                )
                            ...
                            # Get XML for broadcasting to clients
                            scene_xml = get_builtin_scene(builtin_scene)
                            if scene_xml:
                                # Broadcast to clients (simulation already loaded above)
                                await self.broadcast_scene_update(
                                    scene_xml, load_into_simulation=False
                                )
                                response_payload["scene_xml"] = scene_xml
                        ...
                        if self.llm_generator:
                            try:
                                ...
                                if scene_result.get("success") and scene_result.get("scene_xml"):
                                    response_payload["scene_xml"] = scene_result["scene_xml"]
                                    actions.append("generated_llm_scene")
                                    message = scene_result.get(
                                        "message", "Scene generated using LLM"
                                    )
                                else:
                                    return {
                                        "success": False,
                                        "error": scene_result.get(
    ...
    async def broadcast_scene_update(self, scene_xml: str, *, load_into_simulation: bool = True):
        ...
        message = {"type": "scene_updated", "xml": scene_xml}
